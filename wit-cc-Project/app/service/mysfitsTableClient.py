...
def getAllMysfits():
    ...
    for item in response["Items"]:
        ...
        mysfit["feedcount"] = item["FeedCount"]["N"]
...
def getMysfit(mysfitId):
    ...
    mysfit["dislikes"] = item["Dislikes"]["N"]
...
# increment the number of dislikes for a mysfit by 1
def dislikeMysfit(mysfitId):

    # Use the DynamoDB API UpdateItem to increment the number of dislikes
    # the mysfit has by 1 using an UpdateExpression.
    response = client.update_item(
        TableName='MysfitsTable',
        Key={
        ...
        UpdateExpression="SET Dislikes = Dislikes + :n",
        ExpressionAttributeValues={':n': {'N': '1'}}
    )
...
def feedMysfit(mysfitId):
    ...
    response = client.update_item(
        ...
        UpdateExpression="SET FeedCount = FeedCount + :n",
        ExpressionAttributeValues={':n': {'N': '1'}}
    )
